...
export class SpeechService {
    ...
    onInit(): void {
        ...
        this.loadVoices()
            .subscribe(voices => {
                this.voicesAvailable = voices
                    .sort((a: SpeechSynthesisVoice, b: SpeechSynthesisVoice) => a.lang.localeCompare(b.lang))
                let text = localStorage.getItem(localStorageKey)
                if (text) {
    ...
    defaultSettings(): SpeechSettings {
        let voiceURIs = preferredVoices.reduce((map: SelectedVoicesMap, pv: PreferredVoiceURI) => {
    ...
    loadVoices(): Observable<SpeechSynthesisVoice[]> {
        return Observable.create((observer: Observer<SpeechSynthesisVoice[]>) => {
            try {
    ...
    speakMulti(text: string, lang: string, options?: SpeakOptions): Observable<any> {
        if (!this.isSynthesisSupported()) {
        ...
        let matches = text.match(/.+?[.!?]+\s+/g) || [text]
        ...
        return Observable.from(matches)
            ...
            .concatMap(phrase => this.speakObservable(phrase, lang, options)
                .delay(options.pause))
    }

    speakSingle(text: string, lang: string, options?: SpeakOptions): Observable<any> {
        if (!this.isSynthesisSupported()) {
            throw new Error('speech synthesis not supported')
        }
        this.cancel()
    ...
    speakObservable(text: string, lang: string, options: SpeakOptions = {}): Observable<any> {
        ...
        return Observable.create((observer: Observer<any>): any => {
            ...
            try {
                let voice = this.selectVoice(lang)
                ...
                // this.utterance.addEventListener('start', (e: any) => {
                //     console.log('start', this.utterance.text)
                // })

                let onEndHandler = (e?: any) => {
    ...
    selectVoice(lang: string): SpeechSynthesisVoice {
        let langRegExp = new RegExp(`^${lang}-`, 'i')

        let fallbackVoices = this.voicesAvailable
            .filter(voice => langRegExp.test(voice.lang) || langRegExp.test(ANDROID_LANG_MAPPINGS[voice.lang]))

        let prefVoiceURI = this.settings.preferredVoices[lang]
