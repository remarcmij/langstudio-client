...
export class LangHelperID extends LangHelper {
  ...
  getWordVariations(word: string): string[] {
    ...
    this._getVariations(word, variations, mePrefixed)
    return uniq(variations)
  }

  private _getVariations(word: string, variations: string[], mePrefixed: boolean) {
    ...
    if (match) {
      word = match[1]
      this._getVariations(match[1], variations, mePrefixed)
    }
    ...
    if (match && !mePrefixed) {
      ...
      if (!mePrefixed) {
        meWord = this._prefixWithMeng(matchWord)
        mePrefixed = true
        if (meWord !== matchWord) {
          this._getVariations(meWord, variations, mePrefixed)
        }
      }
      ...
      if (match && !mePrefixed) {
        mePrefixed = true
        this._getVariations('mem' + matchWord, variations, mePrefixed)
      }
    }
    ...
    if (match && !mePrefixed) {
      meWord = this._prefixWithMeng(word)
      mePrefixed = true
      if (meWord !== word) {
        this._getVariations(meWord, variations, mePrefixed)
      }
    ...
    if (match && !mePrefixed) {
      mePrefixed = true
      this._getVariations('mem' + word, variations, mePrefixed)
    }
  ...
  private _prefixWithMeng(word: string): string {
    if (word.match(/^[aeiou]/)) {
      word = 'meng' + word
